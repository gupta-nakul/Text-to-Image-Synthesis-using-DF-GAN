...
def parse_args():
    ...
    parser.add_argument(
        "--multi_gpus", type=bool, default=False, help="if use multi-gpu"
    )
...
def sample_example(wordtoix, netG, text_encoder, args):
    ...
    with torch.no_grad():
        ...
        for i in tqdm(range(caption_num)):
            ...
            vutils.save_image(
                fakes.data, img_name, nrow=4, value_range=(-1, 1), normalize=True
            )
...
def main(args):
    ...
    if (args.multi_gpus == True) and (get_rank() != 0):
        ...
        print("*" * 40)
        print(
            "Sampling done, %.2fs cost, saved to %s"
            % (end_t - start_t, args.samples_save_dir)
        )
        print("*" * 40)
