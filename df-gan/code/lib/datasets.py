...
def get_fix_data(train_dl, test_dl, text_encoder, args):
    fixed_image_train, fixed_word_train, fixed_sent_train = get_one_batch_data(
        train_dl, text_encoder, args
    )
    fixed_image_test, fixed_word_test, fixed_sent_test = get_one_batch_data(
        test_dl, text_encoder, args
    )
...
class TextImgDataset(data.Dataset):
    def __init__(self, split="train", transform=None, args=None):
        ...
        self.filenames, self.captions, self.ixtoword, self.wordtoix, self.n_words = (
            self.load_text_data(self.data_dir, split)
        )
