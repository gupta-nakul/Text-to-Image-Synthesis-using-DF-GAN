...
def train(dataloader, netG, netD, netC, text_encoder, optimizerG, optimizerD, args):
    ...
    for step, data in enumerate(dataloader, 0):
        ...
        pred_real, errD_real = predict_loss(
            netC, real_features, sent_emb, negtive=False
        )
    ...
    for time in range(times):
        for i, data in enumerate(dataloader):
            ...
            with torch.no_grad():
                ...
                if save_imgs == True:
                    save_single_imgs(
                        fake_imgs, save_dir, time, dl_length, i, batch_size
                    )
                fake = norm(fake_imgs)
                pred = model(fake)[0]
                if pred.shape[2] != 1 or pred.shape[3] != 1:
...
def save_single_imgs(imgs, save_dir, time, dl_len, batch_n, batch_size):
    for j in range(batch_size):
        ...
        im = np.transpose(im, (1, 2, 0))
        im = Image.fromarray(im)
        filename = "imgs_n%06d_gpu%1d.png" % (
            time * dl_len * batch_size + batch_size * batch_n + j,
            get_rank(),
        )
    ...
    if (multi_gpus == True) and (get_rank() != 0):
        ...
        if writer != None:
            fixed_grid = make_grid(
                fixed_results.cpu(), nrow=8, value_range=(-1, 1), normalize=True
            )
